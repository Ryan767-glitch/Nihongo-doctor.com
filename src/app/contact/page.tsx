    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await fetch("https://formsubmit.co/ajax/urutorayoyo@gmail.com", {
                ...
                body: JSON.stringify({
                    "お名前": formDataObj.name,
                    "メールアドレス": formDataObj._replyto,
                    "カテゴリ": formDataObj.category,
                    "メッセージ": formDataObj.message,
                    _subject: `にほんごドクター.com お問い合わせ: ${formDataObj.category}`
                })
            });
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 sm:p-10">
                ...
                {submitStatus === 'success' && (
                    <div className="bg-green-50 border border-green-200 text-green-800 rounded-md p-4 mb-8">
                        <p className="font-medium">✅ お問い合わせありがとうございます。3営業日以内にご返信いたします。</p>
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        <input
                            type="email"
                            id="email"
                            name="_replyto" // Formspree standard format
                            required
                            className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors"
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            onChange={(e) => {
                                ...
                                if (counter) counter.textContent = e.target.value.length.toString();
                            }}
                        ></textarea>
                    ...
                    <div className="bg-gray-50 p-4 rounded-md text-sm text-gray-600 mb-6">
                        個人情報の取り扱いについては<Link href="/privacy" className="text-blue-600 hover:underline">プライバシーポリシー</Link>をご確認ください。
                    </div>

                    <button
                        type="submit"
                        disabled={isSubmitting}
                        className={`w-full sm:w-auto px-8 py-3 rounded-md font-medium text-white transition-all duration-200 flex justify-center items-center gap-2 ${isSubmitting
